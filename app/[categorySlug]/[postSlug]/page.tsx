...
import { getCategoryUrl, getPostUrl } from '@/lib/urls'
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const realCategorySlug = post.categories?.slug

  if (realCategorySlug && realCategorySlug !== categorySlug) {
    return buildMetadata({
      title: `${post.title} | HarryShare`,
...
export default async function CategoryPostDetailPage({ params }: PageProps) {
  const { categorySlug, postSlug } = await params
  const post = await getPostBySlug(postSlug)
    ...
    <main className="bg-cream">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: categoryName, url: `${siteConfig.url}${categoryUrl}` },
          { name: post.title, url: `${siteConfig.url}${postUrl}` },
        ])}
      />
