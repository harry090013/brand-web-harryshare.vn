  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        <Link href="/" className="font-semibold text-">Harry Share</Link>

        {/* Desktop */}
        <nav className="hidden md:flex items-center gap-5 text-">
          {links.map(l => (
      ...
      {open && (
        <div className="md:hidden border-t bg-white">
          <div className="px-4 py-3 space-y-1">
            {links.map(l => (
              <Link key={l.href} href={l.href} onClick={()=>setOpen(false)} className="block py-2.5 text- border-b last:border-0">
                {l.label}
              </Link>
            ))}
            <Link href="/admin" onClick={()=>setOpen(false)} className="block py-2.5 text- text-blue-600">Admin</Link>
          </div>
        </div>
      )}
    </header>
